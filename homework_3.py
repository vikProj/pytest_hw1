...
def find_median_average(data_set: dict[int: dict]) -> dict[str:float]:
    res1, res2 = split_male_female(data_set)
    result = find_median_average_per_gender(res1)
    result.update(find_median_average_per_gender(res2))
    return result


def find_median_average_per_gender(data: dict[str: List[dict]]) -> dict[str:float]:
    """
